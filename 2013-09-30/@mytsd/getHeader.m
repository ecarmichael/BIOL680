function csc_info = getHeader(tsa)

csc_info = [];
for hline = 1:length(tsa.header)
   
    line = strtrim(tsa.header{hline});
    
    if isempty(line) | ~strcmp(line(1),'-') %skip the lines that not providing informaiton
        continue;
    end
