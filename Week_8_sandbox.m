...
[ccf,lags] = xcorr(data1,data2,100,'coeff'); % now a cross-correlation
lags = lags.*(1./Fs); % convert samples to time
% % plot(lags,ccf, 'g'); grid on;

%% "magnitude-squared coherence"  (aka formal definition of coherence)
figure;
subplot(221);
...
% if we double the amplitude the PSD power increases significantly from
% 0.15 to 0.67 and the Cross-spec doubles in value but the frequency
% remains the same.  There is a very minor change in the xcorr but this is
% negligible 
%
% if you drop the SNR then the coherence will drop quite quickly.  with 2x
% the noise there was a slight drop but with 5x there was a massive drop. 
...
% increasing the tvec length to 10s leads to a decrease in the noise within
% the coherence but does not alter the xcorr or the PSD.  This is because
% we have a greater sample size whichg will average out the noise more
% easily for the coherent signal while the varying phase does not see a
% reduction in the coherence noise since the amount of offset is continuous
% with the size of the tvec.  
...
% if you increase the window size from 50ms to 500ms all kinds of craziness
% happen.  The PSD becomes a series of multiple peaks and the coherence is
% very noisey and covers the entire frequency range.  
...
%% Compute the PSDs and Coherence
Fs = 2000; wsize = 2048;
...
% when comparing the C1 and C5 there is a stark difference n the low gamma
% power in the C5 condition but this also may be the result of having fewer
% trials in the HC while the Vs drop[s in the uppers bands with increased
% reward.  There is also a beta peak in the HC in the C1 condition.  

%% teim-freq coherence analysis
figure;
