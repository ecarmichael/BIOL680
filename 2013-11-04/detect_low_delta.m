function evt = detect_low_delta(csc,varargin)
...
ripple_band = [3 4]; % frequency band to use
threshold = 5; % number of SDs above mean to use for detection
Fs = 2000;
...
if isempty(time_window)==1
    ...
    cscR = Restrict(cscL,time_window(1),time_window(2));
end
...
subplot(312)
hold on
plot_range = median(tvec):median(tvec)+2;
