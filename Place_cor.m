function [out1, out2] = Poiss_cor(cell1_id, cell2_id, varargin)
%%
...
gau_SD = .050; % in ms
fig = 0;  % this is a boolean that will display the verification figures of the SDF
extract_varargin;
%% for each of the two neurons, restrict the data to [3200 5650] (the time interval when the rat was running on the track)
 spk_t1 = Data(Restrict(S{cell1_id},3200,5650));
 spk_t2 = Data(Restrict(S{cell2_id},3200,5650));
...
[xcorr1,xbin1] = ccf(spk_poiss_ts_1,spk_poiss_ts_2,binsize,1);
[xcorr2,xbin2] = ccf(spk_ts_1,spk_ts_2,binsize,1);
figure; subplot(211); plot(xbin1,xcorr1);subplot(212); plot(xbin2,xcorr2);
